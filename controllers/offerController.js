...
// Helper function for exchange rates
...
// Calculates line item price based on pricing method
const calculateLineItemPrice = async (itemData, globalMarginPercent, customerPriceList) => {
    ...
    // Priority 1: Use Customer Price List
    if (pricingMethod === "PriceList" && customerPriceList) {
        ...
        if (priceListItem) {
            ...
            if (sourceCurrency !== "USD") {
                basePriceUSD_for_margin_calc *= 1.03; // Apply currency protection
            }
        }
    }
    ...
    if (basePriceUSD_for_margin_calc === 0 && productDetails.supplierOffers?.length > 0) {
        ...
        for (const supOffer of productDetails.supplierOffers) {
            if (supOffer.originalPrice && supOffer.originalCurrency) {
                ...
                let currentSupplierPriceUSDWithProtection = supOffer.originalPrice * supplierRate;
                if (supOffer.originalCurrency !== "USD") {
                    currentSupplierPriceUSDWithProtection *= 1.03;
                }
...
const updateOffer = asyncHandler(async (req, res) => {
    ...
    offer.validityDate = validityDate ?? offer.validityDate;
    offer.terms = terms ?? offer.terms;
    offer.status = status ?? offer.status;
    offer.globalMarginPercent = globalMarginPercent ?? offer.globalMarginPercent;
...
const generateOfferPdf = asyncHandler(async (req, res) => {
    ...
    const clientPhone = offer.client.fullPhoneNumber ||
        (offer.client.phoneCountryCode && offer.client.phoneNumber ?
            `${offer.client.phoneCountryCode} ${offer.client.phoneNumber}` : '');
    ...
    doc.moveDown(0.5);
    doc.text(offer.terms || "");

    doc.end();
    stream.pipe(res);
});

// GET CSV version of offer
