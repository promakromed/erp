...
const exportCsvBtn = document.getElementById("export-csv-btn"); // Added Export Button reference

// Store current search criteria for export
let currentSearchCriteria = null;

// Function to make authenticated API requests (expecting JSON response)
...
searchForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // Store criteria for potential export
    currentSearchCriteria = requestBody;
    console.log("DEBUG: Stored current search criteria for export.", currentSearchCriteria);
    ...
    currentSearchCriteria = null; // Clear stored criteria on error
  } finally {
    hideLoading();
  }
});
...
clearBtn.addEventListener("click", () => {
  ...
  exportCsvBtn.style.display = "none"; // Hide export button
  currentSearchCriteria = null; // Clear stored criteria
});
...
function displayResults(products, suppliers) {
  ...
  exportCsvBtn.style.display = "inline-block"; // Show export button
...
exportCsvBtn.addEventListener("click", async () => {
  ...
  if (!currentSearchCriteria) {
    console.error("DEBUG: No search criteria available for export.");
    showError("Cannot export - please perform a search first.");
    return;
  }

  console.log("DEBUG: Exporting with criteria:", currentSearchCriteria);
  ...
  try {
    ...
    if (!response.ok) {
      let errorMsg = `Export failed: ${response.statusText}`;
      try {
    ...
    console.log("DEBUG: Determined download filename:", filename);

    // Process the response as a blob (CSV data)
    const blob = await response.blob();
    console.log("DEBUG: Received blob for download.");
...
function showLoading() {
  ...
  noResults.style.display = "none";
  errorMessage.style.display = "none";
  exportCsvBtn.style.display = "none"; // Hide export button while loading
}
